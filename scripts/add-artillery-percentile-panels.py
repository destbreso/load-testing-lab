#!/usr/bin/env python3
"""
Script para agregar paneles de percentiles a los dashboards de Artillery
"""
...
def create_artillery_percentile_timeseries_panel(y_position):
    """Crea un panel de time series con p50, p75, p90, p95, p99 para Artillery"""
    return {
        "datasource": {
            "type": "influxdb",
            "uid": "influxdb"
  ...
  |> filter(fn: (r) => r["_measurement"] == "artillery_response_time")
...
def create_artillery_percentile_stat_panels(y_position):
    """Crea 5 stat panels pequeños con los valores actuales de cada percentil para Artillery"""
    panels = []
    percentiles = [
        ("p50", "Median", "green", 0),
        ("p75", "75th", "blue", 5),
...
def add_percentile_panels(dashboard_path):
    """Agrega paneles de percentiles a un dashboard de Artillery"""
    ...
    percentile_chart = create_artillery_percentile_timeseries_panel(max_y)
    percentile_chart['id'] = max_id + 1
    
    stat_panels = create_artillery_percentile_stat_panels(max_y + 8)
...
def main():
    ...
    artillery_dashboards = [
        "artillery-telegraf.json",
        "artillery-dashboard-elite.json",
        "artillery-dashboard-pro.json"
    ]
    
    print("\n🚀 Agregando paneles de percentiles a dashboards de Artillery\n")
    print("=" * 60)
    
    for dashboard_name in artillery_dashboards:
