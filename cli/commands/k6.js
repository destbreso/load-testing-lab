...
export default async function runK6(options) {
  ...
  if (projectDir) {
    ...
    console.log(chalk.blue(`Running k6 with project: ${projectDir}`));
    ...
    console.log(
      chalk.blue(`Running k6 with external scenario: ${scenarioFile}`),
    );
    ...
    containerPath = `/k6/scenarios/${scenarioFile}`;
    console.log(
      chalk.blue(`Running k6 with built-in scenario: ${scenarioFile}`),
    );

    dockerArgs = ["run", "--rm", "k6", "run", containerPath];
  }
